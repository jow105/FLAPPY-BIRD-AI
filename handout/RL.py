...
class QNet(object):
	...
	def update(self, last_input, last_choice, last_prediction, crash, scored, playerY, pipVelX):
		...
		if(crash):
			...
			if(new_distY > 2*BIRDHEIGHT and new_distY < PIPEGAPSIZE - BIRDHEIGHT):
				if(new_distY > PIPEGAPSIZE/2):
					if(last_choice == 0):
						reward_nf += 50
					if(last_choice == 1):
						reward_f -= 20
					...
					if(last_choice == 1):
						reward_f += 50
			elif(new_distY >= PIPEGAPSIZE - BIRDHEIGHT):
				if(last_choice == 0):
					reward_nf += 20
				if(last_choice == 1):
					reward_f += -50
		...
		if(len(self.states_withreward) > batch):
			indices = random.sample(xrange(len(self.states_withreward)),mini_batch)
			x = []
			y = []
			...
			self.model.fit(np.asarray(x),np.asarray(y),batch_size=mini_batch,epochs=1, verbose=0)
