...
class QNet(object):
	...
	def update(self, last_input, last_choice, last_prediction, crash, scored, playerY, pipVelX):
		...
		if(crash):
			...
			if(last_choice == 1):
				reward_f += 100
		elif(new_distY > PIPEGAPSIZE/2):
			if(last_choice == 0):
				reward_nf += 30
			if(last_choice == 1):
				reward_f += -100
		elif(new_distY > (PIPEGAPSIZE/2)/2 and new_distY <= PIPEGAPSIZE/2):
			if(last_choice == 0):
				reward_nf += -10
			if(last_choice == 1):
				reward_f += 10
		elif(new_distY > 0 and new_distY <= (PIPEGAPSIZE/2)/2):
			if(last_choice == 0):
				reward_nf += -15
			if(last_choice == 1):
				reward_f += 15
		elif(new_distY <= 0 and new_distY > -1*(PIPEGAPSIZE/2)/2):
			if(last_choice == 0):
				reward_nf += -20
			if(last_choice == 1):
				reward_f += 20
		elif(new_distY <= -1*(PIPEGAPSIZE/2)/2 and new_distY > -1*(PIPEGAPSIZE/2)):
			if(last_choice == 0):
				reward_nf += -30
			if(last_choice == 1):
				reward_f += 30
		elif(new_distY <= -1*(PIPEGAPSIZE/2) and new_distY > -1*PIPEGAPSIZE):
			if(last_choice == 0):
				reward_nf += -40
			if(last_choice == 1):
				reward_f += 40
		elif(new_distY <= -1*PIPEGAPSIZE):
			if(last_choice == 0):
				reward_nf += -50
			if(last_choice == 1):
				reward_f += 50

		if(new_distY <= -1*(PIPEGAPSIZE/2) and new_distY > -1*(PIPEGAPSIZE))
